...
def plot_training(history, model_name):
    ...
    plt.title('ResNet Model Doğruluk ve Kayıp Eğrileri')
...
model_ResNet_train = tf.keras.models.Model(inputs=base_model.inputs, outputs=prediction)
 
optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=1e-05)
model_ResNet_train.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])

logfile = "plots/" + "ResNet50" + "_training_output.txt"

start_train = datetime.now()
history = model_ResNet_train.fit(train_ds, validation_data = validate_ds, epochs=epoch,shuffle=True,callbacks=[SaveLogs(logfile)])                                                                                            
end_train = datetime.now()

plot_training(history,'ResNet50')

print("Training duration : {}".format(end_train-start_train))

start_test = datetime.now()
result_final = model_ResNet_train.evaluate(test_ds)
...
with open("plots/" + "ResNet50" + "_training_output.txt", "a") as f:
...
predict = model_ResNet_train.predict(test_ds)
...
plt.title("ResNet50 Karışıklık Matrisi", fontsize=20)
...
plt.savefig('plots/' + 'ResNet50' + '_matrix.png')
