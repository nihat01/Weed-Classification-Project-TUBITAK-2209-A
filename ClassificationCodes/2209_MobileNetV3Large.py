...
def plot_training(history, model_name):
    ...
    plt.title('MobileNetV3Large Model Doğruluk ve Kayıp Eğrileri')
...
base_model = MobileNetV3Large(weights= "imagenet", include_top=False, input_shape=(image_width, image_height, 3))
...
model_mobile_train = tf.keras.models.Model(inputs=base_model.inputs, outputs=prediction)
 
optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=1e-05)
model_mobile_train.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])

logfile = "plots/" + "MobileNetV3Large" + "_training_output.txt"

start_train = datetime.now()
history = model_mobile_train.fit(train_ds, validation_data = validate_ds, epochs=epoch,shuffle=True,callbacks=[SaveLogs(logfile)])                                                                                            
end_train = datetime.now()

plot_training(history,'MobileNetV3Large')

print("Training duration : {}".format(end_train-start_train))

start_test = datetime.now()
result_final = model_mobile_train.evaluate(test_ds)
...
with open("plots/" + "MobileNetV3Large" + "_training_output.txt", "a") as f:
...
predict = model_mobile_train.predict(test_ds)
...
plt.title("MobileNetV3Large Karışıklık Matrisi", fontsize=20)
...
plt.savefig('plots/' + 'MobileNetV3Large' + '_matrix.png')
